...
export async function generateQuestions(
  ...
  try {
    ...
    console.log(`[GENERATOR] Calling OpenAI API with model: ${model}`);
    ...
    // Debug log the beginning of the response
    console.log('[GENERATOR] Response starts with:', content.substring(0, 50));
    
    // Improved JSON parsing with better error handling
    try {
      // First attempt: Try to parse the entire content as JSON
      try {
        const generatedQuestions = JSON.parse(content) as GeneratedQuestion[];
        console.log('[GENERATOR] Successfully parsed response as JSON directly');
        ...
        console.log('[GENERATOR] Direct JSON parse failed, trying to extract JSON array');
        
        // Clean the content - remove any text before the first [ and after the last ]
        const cleanedContent = content.replace(/^[\s\S]*?(\[[\s\S]*\])[\s\S]*$/, '$1');
        console.log('[GENERATOR] Cleaned content starts with:', cleanedContent.substring(0, 50));
        
        try {
          // Try parsing the cleaned content
          const generatedQuestions = JSON.parse(cleanedContent) as GeneratedQuestion[];
          console.log('[GENERATOR] Successfully parsed cleaned content');
          ...
          console.log('[GENERATOR] Cleaned content parse failed, trying regex extraction');
          
          // Second attempt: Find JSON array in the response with a more robust regex
          const jsonMatch = content.match(/(\[[\s\S]*?\{[\s\S]*?\}[\s\S]*?\])/);
          if (!jsonMatch) {
            console.error('[GENERATOR] Could not find JSON array in response');
            console.log('[GENERATOR] Response preview:', content.substring(0, 500));
            throw new Error('Failed to parse generated questions - no JSON array found');
          }
          
          // Clean the extracted JSON string
          let jsonString = jsonMatch[0];
          // Remove any potential trailing or leading non-JSON content
          jsonString = jsonString.trim();
          
          // Parse the extracted JSON
          const generatedQuestions = JSON.parse(jsonString) as GeneratedQuestion[];
          console.log('[GENERATOR] Successfully parsed with regex extraction');
      ...
      console.error('[GENERATOR] All JSON parsing attempts failed:', error);
      console.log('[GENERATOR] Response preview:', content.substring(0, 500));
      
      // Last resort - try to fix the most common JSON issues
      try {
        console.log('[GENERATOR] Attempting to fix malformed JSON');
        
        // Remove any markdown formatting or text descriptions
        let fixedContent = content
          .replace(/^\s*Here are.*?:/s, '') // Remove any introduction text
          .replace(/\d+\.\s+\*\*[^*]+\*\*/g, '') // Remove numbered items with bold markdown
          .replace(/```json|```/g, '') // Remove markdown code fences
          .trim();
        
        // If it doesn't start with [ but contains JSON objects, wrap it in array brackets
        if (!fixedContent.startsWith('[') && fixedContent.includes('{')) {
          fixedContent = '[' + fixedContent;
        }
        if (!fixedContent.endsWith(']') && fixedContent.includes('}')) {
          fixedContent = fixedContent + ']';
        }
        
        console.log('[GENERATOR] Fixed content starts with:', fixedContent.substring(0, 50));
        
        const generatedQuestions = JSON.parse(fixedContent) as GeneratedQuestion[];
        console.log('[GENERATOR] Successfully parsed fixed content');
        ...
        console.error('[GENERATOR] All parsing attempts failed completely:', fixError);
